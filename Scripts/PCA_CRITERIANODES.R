...
MCA <- read.csv("C:/Users/wwainwright/Documents/R/MCDA/Data/PCA/PCA3UnWeighted.csv")
MCA1 <- MCA[c(2:6)]
...
ls(pca) 
  K <- summary(pca)
  K
  write.xlsx(K, file="PCASummary_Nodes.csv")
  G <- pca$sdev^2  # These are be the eigenvalues
  G
  write.xlsx(G, file="PCASummary_NodesEigen.csv")
pca$scale
pca$center
print(pca)
plot(pca, type = "l")
...
g <- autoplot(prcomp(MCA1), main = "Plot B: Criteria nodes", data = MCA, loadings = TRUE, loadings.colour = 'red',
              label=TRUE, label.label="Breed.Score", label.size = 4, loadings.label = TRUE, loadings.label.size  = 4, frame = TRUE,
